...
func registerTools(s *server.MCPServer) {
	...
	s.AddTool(mcp.NewTool("talos_get",
		...
		mcp.WithString("node", mcp.Description("Target node IP or hostname. Single node per call — to fan out across multiple nodes, issue one tool call per node (talosctl --nodes a,b style multi-targeting is not supported here).")),
	...
	s.AddTool(mcp.NewTool("talos_image_remove",
		...
		mcp.WithString("namespace", mcp.Description("Containerd namespace: cri (default, Kubernetes workloads), system (etcd, kubelet)")),
		mcp.WithString("node", mcp.Description("Target node IP or hostname. Single node per call — to fan out across multiple nodes, issue one tool call per node (talosctl --nodes a,b style multi-targeting is not supported here).")),
		mcp.WithString("context", mcp.Description("Talosconfig context name")),
	), handleImageRemove)
